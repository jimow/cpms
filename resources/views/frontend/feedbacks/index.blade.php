        ...
        <div class="col-md-12">
            @can('feedback_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.feedbacks.create') }}">
                            ...
                            <tbody>
                                @foreach($feedbacks as $key => $feedback)
                                    <tr data-entry-id="{{ $feedback->id }}">
                                        ...
                                        <td>
                                            @can('feedback_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.feedbacks.show', $feedback->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('feedback_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.feedbacks.edit', $feedback->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('feedback_delete')
                                                <form action="{{ route('frontend.feedbacks.destroy', $feedback->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('feedback_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.feedbacks.massDestroy') }}",
