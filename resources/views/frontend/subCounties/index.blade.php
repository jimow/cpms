        ...
        <div class="col-md-12">
            @can('sub_county_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.sub-counties.create') }}">
                            ...
                            <tbody>
                                @foreach($subCounties as $key => $subCounty)
                                    <tr data-entry-id="{{ $subCounty->id }}">
                                        ...
                                        <td>
                                            @can('sub_county_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.sub-counties.show', $subCounty->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('sub_county_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.sub-counties.edit', $subCounty->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('sub_county_delete')
                                                <form action="{{ route('frontend.sub-counties.destroy', $subCounty->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sub_county_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.sub-counties.massDestroy') }}",
