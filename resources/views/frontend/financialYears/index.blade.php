        ...
        <div class="col-md-12">
            @can('financial_year_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.financial-years.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.financialYear.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'FinancialYear', 'route' => 'admin.financial-years.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                                    ...
                                    <tr data-entry-id="{{ $financialYear->id }}">
                                        ...
                                        <td>
                                            @can('financial_year_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.financial-years.show', $financialYear->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('financial_year_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.financial-years.edit', $financialYear->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('financial_year_delete')
                                                <form action="{{ route('frontend.financial-years.destroy', $financialYear->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('financial_year_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.financial-years.massDestroy') }}",
